...
uses
  ...
  PythonEngine, FileUtil, Forms, StrUtils;
...
function TNNTrainning.EscapeJSON(const S: string): string;
begin
  Result := S;
...
begin
  Result := '';
  if (AFieldName <> '') and Assigned(dmbase) and Assigned(dmbase.zqryAux) and
     (dmbase.zqryAux.FindField(AFieldName) <> nil) then
...
begin
  ...
  begin
    ...
    begin
      resultado := FsqlTrainning.SQL + ' WHERE ' + FFilterValue +
                   ' like ''' + FFilterCondition + '''';
...
begin
  ...
  begin
    resultado := FsqlTester.SQL + ' WHERE ' + FFilterValueTester +
                 ' like ''' + FFilterConditionTester + '''';
...
begin
  ...
  if Trim(Fjsontrainning) = '' then
  begin
    ShowMessage('JSON de treinamento vazio.');
    Exit;
  end;

  if Trim(FPython) = '' then
  begin
    ShowMessage('Código Python vazio.');
    Exit;
  end;
...
begin
  ...
  jsonStr := jsonStr + '{ "name":"' + EscapeJSON(FNome) +
          '", "comment":"' + EscapeJSON(FComentario) +
          '", "sql":"' + EscapeJSON(IfThen(Assigned(FsqlTrainning), FsqlTrainning.SQL, '')) +
          '", "sqltester":"' + EscapeJSON(IfThen(Assigned(FsqlTester), FsqlTester.SQL, '')) +
          '", "inputfield":"' + EscapeJSON(FInputField) +
          '", "inputref":"' + EscapeJSON(FInputRef) +
          '", "inputreffield":"' + EscapeJSON(FInputRefField) +
          '", "inputfieldtester":"' + EscapeJSON(FInputFieldTester) +
          '", "inputreftester":"' + EscapeJSON(FInputRefTester) +
          '", "inputreffieldtester":"' + EscapeJSON(FInputRefFieldTester) +
          '", "outputfield":"' + EscapeJSON(FOutputField) +
          '", "outputfieldtester":"' + EscapeJSON(FOutputFieldTester) +
          '", "filtercondition":"' + EscapeJSON(FFilterCondition) +
          '", "filterconditiontester":"' + EscapeJSON(FFilterConditionTester) +
          '", "InputCols":"' + IntToStr(FvalinputLeg.Count) +
          '", "OutputCols":"' + IntToStr(FvaloutputLeg.Count) +
          '", "dtatual":"' + EscapeJSON(dataatual) + '" }' + #13;
  ...
  begin
    inputLegLine := inputLegLine + '"' + EscapeJSON(FvalinputLeg.Strings[j]) + '"';
  ...
  begin
    outputLegLine := outputLegLine + '"' + EscapeJSON(FvaloutputLeg.Strings[j]) + '"';
  ...
  begin
    ...
    begin
      if valinput.Cells[j, i].IsEmpty then
        ...
        inputLine := inputLine + '"' + EscapeJSON(valinput.Cells[j, i]) + '"';
    ...
    begin
      if valoutput.Cells[j, i].Trim.IsEmpty then
        ...
        outputLine := outputLine + '"' + EscapeJSON(valoutput.Cells[j, i]) + '"';
...
begin
  ...
  try
    ...
    Output := '';
    filtroExecucao := Trim(FFilterCondition);
    if filtroExecucao = '' then
      filtroExecucao := GetMarcaDiretorio;

    if Callprg('python.exe', pythonfile + ' ' + jsonfile + ' ' + filtroExecucao, Output) then
      ShowMessage(Output);
  except
    on E: Exception do
...
begin
  ...
  if (Trim(FInputRefKey) = '') or (Trim(FInputRef) = '') or (Trim(FInputRefField) = '') then
  begin
  ...
  begin
    ...
    dmbase.zqryAux2.SQL.Text := Format(
      'SELECT %s FROM %s WHERE %s = :palavra',
...
begin
  ...
  begin
    ...
    if (posicaoinput = -1) and (valorinput <> '0') and (Trim(valorinput) <> '') then
...
begin
  ...
  sqlGerado := GeraSQLTrainning;
  if Trim(sqlGerado) = '' then
    Exit;
  ...
  try
    ...
    dmbase.zqryAux.SQL.Text := sqlGerado;
...
begin
  ...
  if Trim(marca) = '' then
    marca := GetMarcaDiretorio;

  sqlGerado := GeraSQLTester;
  if Trim(sqlGerado) = '' then
    Exit;
  ...
  try
    ...
    arquivo.SaveToFile(FfileJSONTester);
...
var
  item: TSQLEditItem;
begin
  if Assigned(Fsetproject) and (query_index >= 0) and
     (query_index < Fsetproject.Querycount) then
  begin
