...
begin
  ...
  MemoryDir := IncludeTrailingPathDelimiter(FWorkspaceRoot) + '.mnote2' +
    PathDelim + 'ai-memory';
...
var P: TMNoteAIProfile;
  ...
  begin
    P := MNoteAI.Profile(R);
    Result := Trim(P.Config.ModelName);
    if Result = '' then Result := Fallback;
  end;
begin
  FRouter.RegisterAgent('source-light', ModelFor(airLightWork, 'light-work'),
    'source', 'bug_fix,small_work,refactor', 'pascal,*', 'lazarus,*',
    0.72, 0.65, 0.15, False);
...
function TMNoteMultiAgentService.Qualify(const AInstruction,
  AFileName: string): TAITaskQualification;
var T, Ext: string;
begin
  FillChar(Result, SizeOf(Result), 0);
  T := LowerCase(AInstruction + ' ' + AFileName);
  Ext := LowerCase(ExtractFileExt(AFileName));
  Result.Domain := 'source';
  Result.SubDomain := 'general';
  Result.TaskType := 'bug_fix';
  Result.Scope := 'single_file';
  Result.Complexity := 0.45;
  Result.Risk := 0.35;
  Result.RequiredCapabilities := 'source_edit,build';
  if Ext = '.pas' then Result.Language := 'pascal'
  else if Ext = '.sql' then Result.Language := 'sql'
  else Result.Language := '*';
  if (Ext = '.pas') or HasAny(T, ['lazarus','free pascal','fpc','form','lfm']) then
    Result.Framework := 'lazarus'
  else Result.Framework := '*';

  if HasAny(T, ['postgres','sql','banco','database','zeos','query','select ',
    'insert ','update ','delete ','transaction','rollback','commit']) then
  begin
    Result.Domain := 'database';
    Result.SubDomain := 'database_source';
    Result.RequiredCapabilities := 'database_reasoning,source_edit,build';
    Result.Complexity := Result.Complexity + 0.15;
    Result.Risk := Result.Risk + 0.20;
    if HasAny(T, ['transaction','rollback','commit','migration','alter table']) then
      Result.TaskType := 'transaction'
    else Result.TaskType := 'database';
    if HasAny(T, ['postgres']) then Result.Framework := 'postgresql';
  end;

  if HasAny(T, ['arquitetura','architecture','projeto','project','refator',
    'múltipl','multipl','vários arquivos','varios arquivos','pacote','package']) then
  begin
    Result.Domain := 'project';
    Result.SubDomain := 'architecture';
    Result.TaskType := 'architecture';
    Result.Scope := 'multi_file';
    Result.RequiredCapabilities := 'project_reasoning,source_edit,build,test';
    Result.Complexity := Result.Complexity + 0.25;
    Result.Risk := Result.Risk + 0.15;
  end;

  if HasAny(T, ['thread','concorr','mutex','critical section','ponteiro','pointer',
    'memória','memory','segmentation','deadlock','generics','generic']) then
  begin
    Result.Complexity := Result.Complexity + 0.30;
    Result.Risk := Result.Risk + 0.25;
  end;
  if HasAny(T, ['só caption','so caption','texto do botão','texto do botao',
    'rename','renomear','comentário','comentario']) then
  begin
    Result.Complexity := 0.20;
    Result.Risk := 0.15;
  end;
  if Result.Complexity > 1 then Result.Complexity := 1;
  if Result.Risk > 1 then Result.Risk := 1;
end;

function TMNoteMultiAgentService.SelectAgent(const AInstruction,
  AFileName: string; out ADecision: TMNoteMultiAgentDecision): Boolean;
var M: TAIDeterministicAgentMemory;
begin
  FLastError := '';
  FillChar(ADecision, SizeOf(ADecision), 0);
  ...
  if not Result then
  begin
    FLastError := ADecision.Route.Reason;
    Exit;
  end;
  ...
  try
    ADecision.SimilarEvidence := M.SimilarEvidenceCount(ADecision.Qualification);
  finally M.Free; end;
  FLastDecision := ADecision;
end;

function TMNoteMultiAgentService.BoolJSON(O: TJSONObject; const N: string;
  D: Boolean): Boolean;
begin if (O <> nil) and (O.Find(N) <> nil) then Result := O.Booleans[N] else Result := D; end;
function TMNoteMultiAgentService.FloatJSON(O: TJSONObject; const N: string;
  D: Double): Double;
begin if (O <> nil) and (O.Find(N) <> nil) then Result := O.Floats[N] else Result := D; end;
function TMNoteMultiAgentService.IntJSON(O: TJSONObject; const N: string;
  D: Integer): Integer;
...
function TMNoteMultiAgentService.ReviewAndRecord(const AInstruction,
  AExecutionOutput: string; ABuildPassed, ATestsPassed: Boolean;
  ARetries: Integer): Boolean;
var P: TMNoteAIProfile; Response, Err, Prompt: string; D: TJSONData; O: TJSONObject;
begin
  FLastError := '';
  FLastReviewText := '';
  FillChar(FLastReview, SizeOf(FLastReview), 0);
  P := MNoteAI.Profile(airArbiter);
  Prompt := 'Você é o supervisor forte. Avalie a execução de um agente especialista.' + LineEnding +
    'Tarefa: ' + AInstruction + LineEnding +
    'Agente: ' + FLastDecision.Route.AgentId + ' / modelo: ' + FLastDecision.Route.ModelId + LineEnding +
    'Resultado: ' + AExecutionOutput + LineEnding +
    'Build passou: ' + BoolToStr(ABuildPassed, True) + LineEnding +
    'Testes passaram: ' + BoolToStr(ATestsPassed, True) + LineEnding +
    'Retorne SOMENTE JSON: {"approved":true,"reviewer_score":0..100,' +
    '"failure_kind":"","corrections_required":0,"should_escalate":false,' +
    '"should_train":false,"notes":"..."}.';
  ...
  try
    D := GetJSON(Response);
    if not (D is TJSONObject) then begin FLastError := 'Supervisor não retornou objeto JSON.'; Exit(False); end;
    O := TJSONObject(D);
    ...
    FSupervisor.RecordReviewedExecution(FLastDecision.Qualification,
      FLastDecision.Route.AgentId, FLastDecision.Route.ModelId,
      FLastReview.Approved, ABuildPassed, ATestsPassed, ARetries, FLastReview);
    FLastReviewText := Response;
    Result := True;
  except
    on E: Exception do begin FLastError := E.Message; Result := False; end;
  end;
  D.Free;
end;
...
begin
  with FLastDecision do
    Result := 'Agente escolhido: ' + Route.AgentId + LineEnding +
      'Modelo: ' + Route.ModelId + LineEnding +
      'Score final: ' + FormatFloat('0.0', Route.Score) + LineEnding +
      ...
      'Tipo: ' + Qualification.TaskType + LineEnding +
      'Complexidade: ' + FormatFloat('0.00', Qualification.Complexity) + LineEnding +
      'Risco: ' + FormatFloat('0.00', Qualification.Risk) + LineEnding +
      'Motivo: ' + Route.Reason;
end;
...
begin
  Result := 'Aprovado: ' + BoolToStr(FLastReview.Approved, True) + LineEnding +
    'Nota: ' + FormatFloat('0.0', FLastReview.ReviewerScore) + LineEnding +
    'Falha: ' + FLastReview.FailureKind + LineEnding +
    ...
    'Priorizar treino: ' + BoolToStr(FLastReview.ShouldTrain, True) + LineEnding +
    'Notas: ' + FLastReview.Notes;
end;
